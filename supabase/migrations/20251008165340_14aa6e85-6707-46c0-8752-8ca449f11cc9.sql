...
-- Guarded: references hardcoded project and user UUIDs from the original DB.
-- On a fresh database those projects/users don't exist so we skip silently.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.project_tasks LIMIT 0  -- just check table exists
  ) THEN
    RAISE NOTICE 'Skipping 20251008165340: project_tasks table not ready.';
    RETURN;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.projects WHERE id = '51525999-41cc-4117-8c25-67bb636a3925'
  ) THEN
    RAISE NOTICE 'Skipping 20251008165340: seed projects not present, skipping task seed data.';
    RETURN;
  END IF;
